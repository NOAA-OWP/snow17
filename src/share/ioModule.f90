...
contains
  ...
  subroutine read_snow17_parameters(this, param_file_name, runinfo)
    ...
    if (ios /= 0) then
      ...
      call write_log('Error opening file : ' // param_file_name, "FATAL")
    endif
 
    print*, 'Reading Snow17 parameters'
    call write_log("ioModule - Reading Snow17 parameters", "INFO")
        ...
        select case (param)
         case ('hru_id')
          ...
          case default
            ...
            call write_log('ioModule - Parameter ' // param // ' not recognized in snow file', 'ERROR')
        end select
    ...
    if(n_params_read /= 26) then
      ...
      call write_log("ioModule - Read " // itoa(n_params_read) // " SNOW17 params, but need 26.  Quitting.", "FATAL")
    end if
  ...
  SUBROUTINE init_forcing_files(namelist, runinfo, parameters)
    ...
    call write_log("ioModule - Initializing forcing files", "INFO")
    found_start = 0
    do nh=1, runinfo%n_hrus
      ...
      if (.not. lexist) then
         call write_log('Problem !! File ' // trim(filename) // 'does not exists.  Quitting.', "FATAL")
         write(*,'(/," ***** Problem *****")')
      ...
      if (ierr /= 0) then
         call write_log("Problem opening file " //trim(filename) //" Error Exiting", "FATAL")
      ...
      if(nh .eq. 1) then
        ...
        call write_log('ioModule -- skipped ' // itoa(skipcount) // ' inaitial records in forcing files', 'INFO')
      endif 
    ...
    if (found_start /= runinfo%n_hrus) then
      call write_log('ERROR: found the starting date in only' // itoa(found_start )// ' out of' // itoa( runinfo%n_hrus) // 'forcing files.', 'FATAL')
  ...
  subroutine read_areal_forcing(namelist, parameters, runinfo, forcing)
    ...
    do nh=1, runinfo%n_hrus
      ...
      if(ierr /= 0) then
        ...
        call write_log(log_msg // '  STOPPING', "FATAL")       
      ...
      if(forcing_datehr /= runinfo%curr_datehr) then
        ...
        call write_log(log_msg // " STOPING", "FATAL")
  ...
  SUBROUTINE init_output_files(namelist, runinfo, parameters)
    ...
    call write_log("Initializing output files", "INFO")
    ...
    if (ierr /= 0) then
      call write_log("Problem opening file " // trim(filename) //". EXITING", "ERROR")      
      write(*,'("Problem opening file ''", A, "''")') trim(filename)
      stop ":  ERROR EXIT"
    endif
  ...
  SUBROUTINE init_new_state_files(namelist, runinfo, parameters)
    ...
    call write_log('Initializing new restart files', 'INFO')
    ...
    if (namelist%write_states == 1) then
      ...
      do nh=1, runinfo%n_hrus
        ...
        if (ierr /= 0) then
          call write_log("Problem opening file " // trim(filename) //". EXITING", "FATAL")      
          write(*,'("Problem opening file ''", A, "''")') trim(filename)
          stop ":  ERROR EXIT"
        endif
  ...
  subroutine write_snow17_statefile(runinfo, namelist, modelvar, n_curr_hru)
    ...
    if(ierr /= 0) then
      call write_log("ERROR writing state file information for sub-unit " // itoa(n_curr_hru) // ". STOPPING", "ERROR")      
  ...
  subroutine read_snow17_statefiles (modelvar, namelist, parameters, runinfo) 
    ...
    call write_log('Reading restart files', 'INFO')
    ...
    call write_log(" -- state datehr: " // trim(state_datehr), "INFO")
    ...
    do hru=1, runinfo%n_hrus
      ...
      if (ios /= 0) then
         ...
         call write_log("Problem opening file " //trim(state_filename), "ERROR")
      endif
    ...
    if (states_found /= runinfo%n_hrus) then 
      call write_log("ERROR:  matching state not found in sac restart file.  Looking for state date: " // trim(state_datehr), "ERROR")
      call write_log("last state read was on: " // trim(statefile_datehr) // ". STOPPING. CHECK INPUTS", "ERROR")
  ...
  SUBROUTINE write_snow17_output(namelist, runinfo, parameters, forcing, modelvar, n_curr_hru)
    ...
    if (namelist%output_hrus == 1 .and. runinfo%n_hrus > 1) then
      ...
      if(ierr /= 0) then
        call write_log('ERROR writing output information for sub-unit ' // itoa(n_curr_hru) // '. STOPPING.', "ERROR")      
    ...
    if (n_curr_hru .eq. runinfo%n_hrus) then 
      ...
      if(ierr /= 0) then
        call write_log("ERROR writing output information for basin average. STOPPING.", "FATAL")      
