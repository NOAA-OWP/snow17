...
! by a single backslash.
  ...
  subroutine parse_date (date, year, month, day, hour, min, sec, error)
    ...
    ! date format length is allowable ... continue
  ...
  ! convert a datestring (YYYYMMDD[HH][MM][SS] to unixtime with origin 1970-1-1
  double precision function date_to_unix (date)
    ...
    ! date is flexible string YYYYMMDD[HH][MM]
  ...
  ! convert unixtime to integer date elements
  subroutine unix_to_date_elem (uxtime, year, month, day, hour, min, sec)
 
    double precision, intent (in) :: uxtime
    integer, intent (out)         :: sec, min, hour, day, month, year
    ! local vars
    integer                       :: u_day, i_day
  ...
  subroutine unix_to_datehr (uxtime, datehr)
    ...
    double precision, intent (in)    :: uxtime
    character (len=10), intent (out) :: datehr
...
! ==== following moved here from day_of_year.f90 =====
